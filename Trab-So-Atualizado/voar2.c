...
void trata_sig_freia(int sinal){
    printf("Freio ocorrendo");
    
      //movimento horizontal(em x)
        if ((mem_aviao->direcao == 'W') && ((mem_aviao->x) < 0.5)) {
            mem_aviao->x += 0.015f; // vem da esquerda para o centro
        } 
        else if ((mem_aviao->direcao == 'E') && ((mem_aviao->x) > 0.5)) {
            
            mem_aviao->x -= 0.015f; // vem da direita para o centro
        }
        //movimento vertical(em y)
        if (mem_aviao->y < 0.5f){
            puts("Y abaixo do aviao!\n");
            mem_aviao->y += 0.005f; // se o aviao iniciar abaixo do aeroporto  
            printf("Pos Y:%f\n",mem_aviao->y);
        }  
        else if (mem_aviao->y > 0.5f){
            puts("Y abaixo do aviao!\n");
            mem_aviao->y -= 0.005f; //se iniciar acima do aeroporto  
            printf("Pos Y:%f\n",mem_aviao->y);
        } 
        //pousando
...
int main(int argc, char* argv[]){
    ...
    signal(SIGUSR2, trata_sig_freia);
